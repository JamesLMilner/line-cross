...
LatLon.prototype.toVector = function() {
    var theta = this.lat.toRadians();
    var lambda = this.lon.toRadians();

    // right-handed vector: x -> 0°E,0°N; y -> 90°E,0°N, z -> 90°N
    var x = Math.cos(theta) * Math.cos(lambda);
    var y = Math.cos(theta) * Math.sin(lambda);
...
Vector3d.prototype.toLatLonS = function() {
    var theta = Math.atan2(this.z, Math.sqrt(this.x*this.x + this.y*this.y));
    var lambda = Math.atan2(this.y, this.x);

    return new LatLon(theta.toDegrees(), lambda.toDegrees());
};
...
LatLon.prototype.greatCircle = function(bearing) {
    ...
    var x =  Math.sin(lambda) * Math.cos(theta) - Math.sin(theta) * Math.cos(lambda) * Math.sin(theta);
    var y = -Math.cos(lambda) * Math.cos(theta) - Math.sin(theta) * Math.sin(lambda) * Math.sin(theta);
    var z =  Math.cos(theta) * Math.sin(theta);
...
LatLon.prototype.destinationPoint = function(distance, bearing, radius) {
    ...
    var x = p1.times(Math.cos(delta));          // component of p2 parallel to p1
    var y = c.cross(p1).times(Math.sin(delta)); // component of p2 perpendicular to p1
...
LatLon.prototype.enclosedBy = function(points) {
    ...
    for (var n=0; n<points.length; n++) {
        ...
        var alpha = c1.angleTo(c2, v); // angle between great-circle vectors, signed by direction of v
        if (alpha < 0) throw new Error('Polygon is not convex');
    }
