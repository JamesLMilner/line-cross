...
LatLon.prototype.toCartesian = function() {
    var theta = this.lat.toRadians(), lambda = this.lon.toRadians();
    ...
    var eSq = (a*a - b*b) / (a*a);
    var nu = a / Math.sqrt(1 - eSq*sintheta*sintheta);

    var x = (nu+h) * costheta * coslambda;
    var y = (nu+h) * costheta * sinlambda;
    var z = ((1-eSq)*nu + h) * sintheta;
...
Vector3d.prototype.toLatLonE = function(datum) {
    ...
    var sintheta = Math.sin(theta), costheta = Math.cos(theta);
    var nu = a/Math.sqrt(1-e2*sintheta*sintheta); // length of the normal terminated by the minor axis
    var h = p*costheta + z*sintheta - (a*a/nu);

    var point = new LatLon(theta.toDegrees(), lambda.toDegrees(), datum);
